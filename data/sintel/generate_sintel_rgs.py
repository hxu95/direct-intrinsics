...
# process all shading images to gray_shading
for i in range(len(paths[ss])):
    print("first %s" %i)
    imgs = {}
    for k in keys:
...
# process all clean images to RGS
...
#get paths for clean image directory definitions ->rgs
paths_clean = glob.glob(join(args.dir_data, 'sources/source_clean_*.txt'))
...
#get paths for shading -> grey shading
paths_shading = glob.glob(join(args.dir_data, 'sources/source_shading_*.txt'))
